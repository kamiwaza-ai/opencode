...
describe("session.llm.stream", () => {
  ...
  it.instance(
    "uses AI SDK generation and skips native runtime when streaming is disabled",
    () =>
      Effect.gen(function* () {
        const model = loadFixture("openai", "gpt-5.2").model
        const request = waitRequest(
        ...
        const failingNativeClient = Layer.succeed(
          LLMClient.Service,
          LLMClient.Service.of({
            prepare: () => Effect.die(new Error("native LLM client should not be used when streaming is disabled")),
            stream: () => Stream.die(new Error("native LLM client should not be used when streaming is disabled")),
            generate: () => Effect.die(new Error("native LLM client should not be used when streaming is disabled")),
          }),
        )

        const resolved = yield* Provider.use.getModel(ProviderV2.ID.openai, ModelV2.ID.make(model.id))
        const sessionID = SessionID.make("session-test-non-stream")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const events = yield* collectWith(
          AppNodeBuilder.build(LLM.node, [
            [LayerNodePlatform.llmClient, failingNativeClient],
            [RuntimeFlags.node, RuntimeFlags.layer({ experimentalNativeLlm: true, streaming: false })],
          ]),
          {
            user: {
              id: MessageID.make("msg_user-non-stream"),
              sessionID,
              role: "user",
              time: { created: Date.now() },
              agent: agent.name,
        ...
        expect(events).toContainEqual(
          expect.objectContaining({ type: "text-delta", text: "Generated without streaming" }),
        )
      }),
    { config: () => openAIConfig(loadFixture("openai", "gpt-5.2").model, `${state.server!.url.origin}/v1`) },
    60_000,
